...
module mypost::transaction_test {
    ...
    fun test_create() {
        ...
        {
            ts::next_tx(&mut ts, ALICE);
            assert!(ts::has_most_recent_for_sender<Transaction>(&ts), 1);
            assert!(ts::has_most_recent_shared<TransactionPool>(), 1);
            let tpool: TransactionPool = ts::take_shared(&ts);
            let number = transaction::get_no_of_accessors(&tpool);
            assert!(number == 1, 1);
            ts::return_shared<TransactionPool>(tpool);
        };
        {
            ts::next_tx(&mut ts, BOB);
            ...
            let tpool: TransactionPool = ts::take_shared(&ts);
            let clock: Clock = clock::create_for_testing(ts::ctx(&mut ts));
            transaction::buy(coin, ADMIN, b"digest", &mut tpool, &clock, ts::ctx(&mut ts));
            ts::return_shared<TransactionPool>(tpool);
            clock::destroy_for_testing(clock);
        };
        {
        ...
        };
        {
            ...
            let tpool: TransactionPool = ts::take_shared(&ts);
            let clock: Clock = clock::create_for_testing(ts::ctx(&mut ts));
            transaction::sell(ADMIN, b"digest", &mut tpool, &clock, ts::ctx(&mut ts));
            ts::return_shared<TransactionPool>(tpool);
            clock::destroy_for_testing(clock);
        };
