...
import { BuyRequest, CommonResponse, ReplyBuyRequest } from "@/lib/shared/interfaces";
import { AccessBought, ReplyAccessBoughtEvent, TransactionCreated } from "@/types/transaction";
import {  REPLY_ACCESS_MUTATE_FALSE_ROUTE, REPLY_ACCESS_MUTATEDB } from "@/lib/api/constant";


const buildTx: GaslessTransactionBytesBuilder = async (req, { wallet }) => {
    const [error, body] = validate(req.body, ReplyBuyRequest);
    ...
    const gaslessTxBytes = await buildGaslessTransactionBytes({
      ...
      build: async (txb) => {
        ...
        txb.moveCall({
            target: `${MYPOST_MOVE_PACKAGE_ID}::reply::buy`,
            arguments: [
...
const parseTxRes: TransactionResponseParser<CommonResponse> = async (req, txRes, user) => {
    ...
    let accessBought = txRes.events?.at(0)?.parsedJson as ReplyAccessBoughtEvent;
    let acessSellBody = {
    ...
    const boughtRes = await fetch(`${API_HOST}${REPLY_ACCESS_MUTATEDB}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
