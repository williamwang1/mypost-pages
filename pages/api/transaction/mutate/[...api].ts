...
import { TransactionRequest, TransactionResponse} from "@/lib/shared/interfaces";
...
const parseTxRes: TransactionResponseParser<TransactionResponse> = async (_, txRes, user) => {
    ...
    const txMetaRes = await fetch(`${API_HOST}/api/transactionmeta/mutatedb`, {
        method: 'POST',
        headers: {
    ...
    const accessRes = await fetch(`${API_HOST}/api/access/bought/mutatedb`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
