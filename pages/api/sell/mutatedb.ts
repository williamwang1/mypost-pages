...
export default async function handler(
    ...
    try {
        ...
        const access: AccessBoughtData | null = await prisma.accessSold.findUnique({
            where: {
                digest: req.body.digest,
            }
        });
        console.log('mutatedb accesssold db ' + JSON.stringify(access))
        if (access) {
            ...
            console.log('in sell mutatedb save start ')
            result = await prisma.accessBought.update({
                where: {
                    digest: req.body.digest
                },
            ...
            result = await prisma.accessSold.create({
                data: {
                    digest: req.body.digest,
                    access_id: req.body.access_id,
                    package_id: req.body.package_id,
            ...
            console.log('in sell mutatedb ' + JSON.stringify(result))
        }
