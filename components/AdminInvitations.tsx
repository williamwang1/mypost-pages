...
const AdminInvitations = () => {
  ...
  const fetchMoreData = () => {
    axios
      ...
      .then((res) => {
        console.log('in admin invitations ' + JSON.stringify(res.data))
        //setItems(res.data)
  ...
  return (
    <div>
        <button className="bg-sky-400 text-white py-2 px-4 rounded-3xl" onClick={handleCreate}>Create</button>
        <InfiniteScroll
        ...
        <div
          role="list"
          className="divide-y divide-gray-100 mt-4 overflow-hidden bg-white 
          shadow-sm ring-1 ring-gray-900/5 rounded-xl"
        >
            {items &&
            items.map((item) => 
